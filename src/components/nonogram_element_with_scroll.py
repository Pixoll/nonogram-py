...
class NonogramElementWithScroll(Element):
    ...
    def __init__(self, show_content: int, nonogram: Nonogram, block_size: int, padding: int):
        super().__init__(show_content, show_content)
    ...
    def update(self):
        ...
        self._nonogram_element.set_position((
            self._nonogram_element.position[0] + scroll_offset,
            self._nonogram_element.position[1]
        ))
